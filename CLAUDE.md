...
每完成一个 Stage，必须更新 `docs/roadmap.md`，参照 Stage 1 的格式：

1. **"已完成"区域**：新增该 Stage 的完成记录，包括：
   - 项目结构变化（如有新文件）
   - 新增/修改的模块说明
   - 关键设计决策
2. **"已掌握 Python"区域**：新增该 Stage 学到的 Python 语法/知识点
3. **"已掌握的软件设计思想"区域**：新增该 Stage 领悟到的设计原则
4. **"下一阶段"区域**：将当前 Stage 标记为 `~~删除线~~ ✅ 完成`
